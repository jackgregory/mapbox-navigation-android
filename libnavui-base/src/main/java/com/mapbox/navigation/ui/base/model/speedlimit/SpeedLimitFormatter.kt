...
class SpeedLimitFormatter(
    ...
    private fun getSpeedLimit(sign: SpeedLimitSign, unit: SpeedLimitUnit, speedLimitKmph: Int): String {
        return if (sign === SpeedLimitSign.VIENNA) {
            if (unit === SpeedLimitUnit.KILOMETRES_PER_HOUR) {
                ...
                val speed = (5 * (speedLimitKmph * SpeedLimitViewConstants.KILO_MILES_FACTOR / 5).roundToInt()).toDouble()
