...
class MapRouteLineTest {
    ...
    fun findDistanceOfPointAlongLineWhenReturningToBeginningOfLine() {
        val lineString = "mjylgAjqguhF??bBmUfCa\\wXsGab@{JelAwXeFkBwXoH}DmAsFkA_@MaMyCuJgCcFmAa\\oHoJ]gC?ePfD_SzKiSfNuNzJiCzA}J`H_NpHuJdEoDvCoHrFgOlJijAto@qGtD{GfDcZdO_OpHgNpHaHtD_EuEViBaCuEpHeE????qHdE`CtEWhB~DtE`HuDfNqH~NqHbZeOzGgDpGuDhjAuo@fOmJnHsFnDwCtJeE~MqH|JaHhC{AtN{JhSgN~R{KdPgDfC?nJ\\`\\nHbFlAtJfC`MxC^LrFjA|DlAvXnHdFjBdlAvX`b@zJvXrG\\uE??"
        ...
        for (index in 0 until 50) {
            val midPoint = TurfMeasurement.midpoint(line.coordinates()[index], line.coordinates()[index + 1])
