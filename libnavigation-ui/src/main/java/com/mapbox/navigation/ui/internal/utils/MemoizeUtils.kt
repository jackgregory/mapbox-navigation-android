...
internal object MemoizeUtils {
    ...
    private class MemoizeHandlerLimited<F, in K : MemoizeCall<F, R>, out R>(val func: F, cacheSize: Int) {
    ...
    private data class MemoizeKey2<out P1, out P2, R>(val p1: P1, val p2: P2) : MemoizeCall<(P1, P2) -> R, R> {
        override fun invoke(func: (P1, P2) -> R) = func(p1, p2)
    }

    fun <P1, P2, R> ((P1, P2) -> R).memoize(cacheSize: Int): (P1, P2) -> R {
        return object : (P1, P2) -> R {
            private val handler = MemoizeHandlerLimited<((P1, P2) -> R), MemoizeKey2<P1, P2, R>, R>(this@memoize, cacheSize)
